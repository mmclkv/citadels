 ...
 *  - 已经被点名的牌（法师看过的牌、逮捕令要没收的牌、正在结算的墓地牌）钉在原位，
 *    不然猜出来的世界会和已经告诉玩家的公开事实自相矛盾、动作走不通；
 *  - 逮捕令只公开三个编号，哪张是真的（signed）按公开集合重新随机指定。
 *
...
function determinize(state, playerId, rng = defaultRng) {
  ...
  redistribute(districtPiles(clone, me, knownCardUids(clone)), rng);
  redistribute(charPiles(clone, me), rng);
  const warrant = clone.effects && clone.effects.magistrate;
  if (warrant && warrant.playerIdx !== me && Array.isArray(warrant.nums) && warrant.nums.length) {
    warrant.signed = warrant.nums[Math.floor(rng() * warrant.nums.length)];
  }
