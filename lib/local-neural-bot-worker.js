...
const { informationSetKey } = require('../training/information-set.js');
 ...
 */
async function decideBySearch({ checkpoint, states, weights, view, playerId, actions, mcts: options,
  deadlineAt }) {
  const active = loadModel(checkpoint);
  // 信念权重：与粒子池等长才作数，长度对不上宁可均匀采样也不要错位
  const belief = Array.isArray(weights) && weights.length === states.length ? weights : null;
  let result;
  try {
    result = await mcts.search({
      rootStates: states,
      particleWeights: belief || undefined,
      ...
      encodeState,
      encodeAction,
      // 真实对局也必须按当前行动者的可见信息拆分节点；否则不同隐藏世界
      // 在同一动作后会错误共享统计量，变成隐式的信息泄漏/策略污染。
      infosetKeyFn: (working, nextPlayerId) => informationSetKey(working, nextPlayerId, Engine.sanitize),
