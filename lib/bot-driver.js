'use strict';

// One outstanding decision per room. Changed game state invalidates old answers.
function summarizeAction(action) {
  if (!action || typeof action !== 'object') return action == null ? null : String(action);
  const out = {};
  Object.keys(action).forEach(key => {
    const value = action[key];
    if (value == null || ['string', 'number', 'boolean'].includes(typeof value)) out[key] = value;
  });
  return out;
}

function createBotDriver({ Engine, AI, agent, neural, currentActor, send, delay, debug = () => {} }) {
  function cancel(r) {
    if (r.botJob) { clearTimeout(r.botJob.timer); r.botJob.controller.abort(); }
    r.botJob = null;
    r.agentStatus = null;
  }
  function tick(r) {
    if (r.botJob || r.closed || !r.state || r.state.phase === 'gameover') return;
    const actor = currentActor(r.state);
    if (!actor || !actor.isBot) { r.agentStatus = null; return; }
    if (r.agentStatus && r.agentStatus.state === 'error' && r.agentStatus.playerId === actor.id) return;
    const job = { controller: new AbortController(), timer: null };
    r.botJob = job;
    job.timer = setTimeout(async () => {
      const st = r.state;
      let stamp;
      let action;
      const modelDriven = actor.botType === 'agent';
      const neuralDriven = actor.botType === 'neural';
      const valid = () => r.botJob === job && !r.closed && r.state === st &&
        currentActor(st) === actor && actor.isBot && (!stamp || JSON.stringify(st) === stamp);
      try {
        if (!valid()) return;
        const available = Engine.getAvailableActions(st, actor.id) || {};
        debug(r, { kind: 'decision_start', playerId: actor.id, playerName: actor.name,
          botType: actor.botType || 'npc', botLevel: actor.botLevel || 'normal',
          phase: st.phase, round: st.round, legalCount: (available.actions || []).length,
          legalActions: (available.actions || []).slice(0, 80).map(summarizeAction),
          state: { gold: actor.gold, hand: actor.hand && actor.hand.length, city: actor.city && actor.city.length } });
        if (modelDriven) {
          r.agentStatus = { playerId: actor.id, state: 'thinking', model: agent.status().model };
          send(r);
          stamp = JSON.stringify(st);
          const result = await agent.decide(st, actor.id, { signal: job.controller.signal });
          if (!valid()) return;
          action = result.action;
          r.agentStatus = { playerId: actor.id, state: 'ready', model: result.model };
        } else if (neuralDriven) {
          r.agentStatus = null;
          stamp = JSON.stringify(st);
          action = await neural.decide(st, actor.id, {
            signal: job.controller.signal,
            mcts: actor.mcts || { simulations: r.config && r.config.mctsSimulations,
              maxDepth: r.config && r.config.mctsMaxDepth,
              particles: r.config && r.config.mctsParticles }
          });
          const neuralStatus = neural.status && neural.status();
          debug(r, { kind: 'decision_detail', playerId: actor.id, playerName: actor.name,
            botType: 'neural', action: summarizeAction(action),
            inference: neuralStatus && neuralStatus.tta && neuralStatus.tta.lastInference,
            checkpoint: neuralStatus && neuralStatus.checkpoint,
            mcts: { simulations: r.config && r.config.mctsSimulations,
              maxDepth: r.config && r.config.mctsMaxDepth, particles: r.config && r.config.mctsParticles } });
          if (!valid()) return;
        } else {
          r.agentStatus = null;
          action = AI.decide(st, actor.id);
          debug(r, { kind: 'decision_detail', playerId: actor.id, playerName: actor.name,
            botType: 'npc', botLevel: actor.botLevel || 'normal', action: summarizeAction(action),
            strategy: '启发式评分与规则优先级' });
        }
        if (!action && !modelDriven) action = (Engine.getAvailableActions(st, actor.id).actions || []).find(a => !a.disabled);
        if (!action) throw new Error('没有可执行的行动');
        debug(r, { kind: 'action_apply', playerId: actor.id, playerName: actor.name,
          botType: actor.botType || 'npc', action: summarizeAction(action) });
        let res = Engine.applyAction(st, actor.id, action);
        if (!res.ok && !modelDriven) {
          const fallback = (Engine.getAvailableActions(st, actor.id).actions || []).find(a => !a.disabled);
          if (fallback) res = Engine.applyAction(st, actor.id, fallback);
          if (!res.ok && st.turn) Engine.applyAction(st, actor.id, { type: st.turn.pending ? 'ability_skip' : 'end_turn' });
        }
        if (!res.ok && modelDriven) throw new Error('模型行动未通过游戏规则校验');
        r.nextBotDelay = delay(r, action);
        send(r);
      } catch (e) {
        if (modelDriven && valid()) {
          r.agentStatus = { playerId: actor.id, state: 'error', model: agent.status().model,
            message: e.code ? e.message : 'Agent 决策失败，请重试或切换普通电脑' };
          send(r);
        } else if (neuralDriven && valid()) {
          console.error('Neural decision failed, falling back to heuristic:', e.message);
          debug(r, { kind: 'decision_error', playerId: actor.id, playerName: actor.name,
            botType: 'neural', error: e.message, fallback: '启发式电脑' });
          try {
            const fallback = AI.decide(st, actor.id) ||
              (Engine.getAvailableActions(st, actor.id).actions || []).find(a => !a.disabled);
            debug(r, { kind: 'fallback_action', playerId: actor.id, playerName: actor.name,
              botType: 'npc', action: summarizeAction(fallback) });
            if (fallback) Engine.applyAction(st, actor.id, fallback);
            r.nextBotDelay = delay(r, fallback);
            send(r);
          } catch (fallbackError) { console.error('Neural fallback failed:', fallbackError.message); }
        } else if (!modelDriven) console.error('NPC decision failed:', e.message);
      } finally {
        if (r.botJob === job) { r.botJob = null; tick(r); }
      }
    }, r.nextBotDelay == null ? (r.config.botPace || 430) : r.nextBotDelay);
    r.nextBotDelay = null;
  }
  return { tick, cancel };
}
module.exports = { createBotDriver };
