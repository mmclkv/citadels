...
function createLocalNeuralBot({
    ...
    return new Promise((resolve, reject) => {
      ...
      try {
        active.postMessage({ type: 'decide', id, checkpoint, view, playerId, actions, variants, deadlineAt,
          states: search ? search.states : null, mcts: search ? search.mcts : null });
      } catch (error) {
        pending.delete(id);
        clearEntry(entry);
        reject(error);
  ...
  /**
   * 生成若干份确定化根局面。只有「合法动作集与真实局面逐字相同」的猜测才能用：
   * 猜测改了合法动作，说明它已经不是当前这个公开局面，丢掉比将就搜更安全。
   */
  ...
  function status() {
    ...
    const mcts = { determinizations, maxSimulations: MCTS_MAX_SIMULATIONS,
  ...
  function decide(state, playerId, { signal, mcts } = {}) {
    ...
    let search = null;
    if (plan) {
      const roots = determinizeRoots(state, playerId, actions, determinizations);
      if (roots.length) search = { states: roots, mcts: plan };
    }
