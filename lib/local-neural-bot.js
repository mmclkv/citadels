...
function createLocalNeuralBot({
  ...
  function ensureNativeClient(checkpoint) {
    const executable = path.join(root, 'native', 'mcts_worker_libtorch' + (process.platform === 'win32' ? '.exe' : ''));
  ...
  function decide(state, playerId, { signal, mcts } = {}) {
    ...
    if (search) {
      try {
        ...
        // 开发环境可能尚未编译 native worker；保留 JS MCTS 作为兼容回退，
        // 正式房间在有 native/mcts_worker_libtorch 时会优先走 LibTorch/GPU。
        if (error && error.code !== 'native_worker_missing') throw error;
      }
    }
