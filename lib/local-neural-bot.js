...
function createLocalNeuralBot({ root = path.join(__dirname, '..') } = {}) {
  ...
  // Game rooms are reproducible: use the versioned repository policy when present.
  // Local checkpoints remain a fallback for development checkouts without that asset.
  ...
  function status() {
    ...
    return { configured: true, checkpoint: checkpoint.name, game: checkpoint.game,
      ...
      profile: loaded && loaded.name === checkpoint.name ? loaded.profile : checkpoint.profile,
