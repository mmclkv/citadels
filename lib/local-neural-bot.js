...
const MCTS_DEFAULT_SIMULATIONS = 500;
const MCTS_DEFAULT_MAX_DEPTH = 700;
const MCTS_MAX_DEPTH_CAP = 700;
...
function summarizeDebugAction(action) {
  if (!action || typeof action !== 'object') return action == null ? null : String(action);
  const out = {};
...
function policyDebugDistribution(actions, policy) {
  return (Array.isArray(actions) ? actions : []).map((action, index) => ({
    action: summarizeDebugAction(action),
    probability: Number(policy && policy[index]) || 0
...
function createLocalNeuralBot({
  ...
  const workerFile = path.join(__dirname, 'local-neural-bot-tta-worker.js');
  ...
  function loadModelPayload(checkpoint) {
    return JSON.parse(zlib.gunzipSync(fs.readFileSync(checkpoint.file)).toString('utf8'));
  }

  function ensureNativeModel(checkpoint) {
    const key = checkpointKey(checkpoint);
    if (nativeModelKey === key && nativeModelPath) return nativeModelPath;
    const payload = loadModelPayload(checkpoint);
    const profile = (payload.config && payload.config.profile) || checkpoint.profile || 'large';
    // 权重本身携带训练时的编码版本；C++ 推理只消费展平后的参数，不能把
    // 新版本硬编码进去，否则旧 checkpoint 会在导入阶段被错误拒绝。
    const encodingVersion = Number(payload.model && payload.model.encodingVersion) || 7;
    const model = new PolicyValueNetwork({ profile, stateSize: 672, actionSize: 256, encodingVersion });
    model.import(payload.model);
    const flat = model.exportFlat();
    const file = path.join(os.tmpdir(), 'citadels-room-policy-' + process.pid + '.bin');
    fs.writeFileSync(file, Buffer.from(flat.buffer, flat.byteOffset, flat.byteLength));
    nativeModelPath = file;
    nativeModelKey = key;
    nativeProfile = profile;
    if (nativeClient) nativeClient.setModelPath(file);
    return file;
  }

  function ensureNativeClient(checkpoint) {
    const executable = nativeWorkerManager
      ? nativeWorkerManager.executable
      : path.join(root, 'native', 'mcts_worker_libtorch' + (process.platform === 'win32' ? '.exe' : ''));
    if (!fs.existsSync(executable)) throw createError('未找到 LibTorch MCTS worker：' + executable, 'native_worker_missing');
    if (nativeWorkerManager && (!nativeWorkerManager.child || nativeWorkerManager.child.exitCode != null)) {
      throw createError('LibTorch MCTS worker 尚未启动或已退出', 'native_worker_unavailable');
    }
    const modelPath = ensureNativeModel(checkpoint);
    if (!nativeClient) {
      nativeClient = new NativeSearchClient({
        root, executable, simulations: MCTS_DEFAULT_SIMULATIONS,
        maxDepth: MCTS_DEFAULT_MAX_DEPTH, batchSize: 32, cPuct: 1,
        seed: 20260913, gpuEvaluator: true, profile: nativeProfile,
        device: 'cuda', inferenceBackend: 'libtorch', searchTimeoutMs: MAX_INFERENCE_MS,
        childProvider: nativeWorkerManager ? () => nativeWorkerManager.child : null,
        restartChild: nativeWorkerManager ? () => nativeWorkerManager.restartSync() : null
      });
    }
    nativeClient.setModelPath(modelPath);
    return nativeClient;
  }
  ...
  function settle(message) {
    ...
    lastInference = { variants: message.variants, timedOut: !!message.timedOut,
      ...
      policy: Array.isArray(message.policy) ? policyDebugDistribution(entry.actions, message.policy) : null };
  ...
  function status() {
    ...
    const mcts = { determinizations, beliefDecay: BELIEF_DECAY,
      defaultSimulations: MCTS_DEFAULT_SIMULATIONS,
      defaultMaxDepth: MCTS_DEFAULT_MAX_DEPTH,
      maxSimulations: MCTS_MAX_SIMULATIONS,
      maxDepthCap: MCTS_MAX_DEPTH_CAP };
    ...
    return { configured: true, checkpoint: checkpoint.name, game: checkpoint.game,
      ...
      message: sourceLabel + '：' + checkpoint.name +
        (checkpoint.game ? '（已训练 ' + checkpoint.game + ' 局）' : ''),
      backend: 'cpp-libtorch', tta, mcts };
  }
  ...
  function decide(state, playerId, { signal, mcts } = {}) {
    ...
    if (search) {
      try {
        const client = ensureNativeClient(checkpoint);
        // 房间配置仍可覆盖默认值；默认值与严格评测一致，但不能让 native
        // worker 把显式的较小搜索预算静默吞掉。
        client.simulations = search.mcts.simulations;
        client.maxDepth = search.mcts.maxDepth;
        client.profile = nativeProfile;
        const startedAt = Date.now();
        return client.search(search.states, playerId, actions, checkpoint.game || 0, search.weights)
          .then(result => {
            if (!result || !result.policy || result.policy.length !== actions.length)
              throw createError('LibTorch MCTS 返回了无效策略分布', 'native_invalid_result');
            let random = Math.random();
            let selected = actions.length - 1;
            for (let i = 0; i < result.policy.length; i++) {
              random -= Number(result.policy[i]) || 0;
              if (random <= 0) { selected = i; break; }
            }
            loaded = { key: checkpointKey(checkpoint), profile: nativeProfile };
            lastInference = { variants: 0, timedOut: false, method: 'native-mcts',
              determinizations: search.states.length, visits: result.visits || 0,
              durationMs: Date.now() - startedAt,
              policy: policyDebugDistribution(actions, result.policy) };
            return actions[selected];
          });
      } catch (error) {
        // 已移除 JS MCTS 回退；native worker 不可用时必须明确报错，避免
        // 用户以为自己在使用搜索，实际却悄悄退化成纯策略网络。
        throw error;
      }
    }
  ...
  function close() {
    ...
    if (nativeClient) nativeClient.close();
    nativeClient = null;
    nativeModelKey = '';
    try { if (nativeModelPath) fs.unlinkSync(nativeModelPath); } catch (_) { /* best effort */ }
    nativeModelPath = '';
  }
