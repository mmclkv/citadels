...
const { sanitizeConfig, STATE_ENCODING_VERSION } = require('../training/train.js');
...
function createTrainingManager({ root = path.join(__dirname, '..'), forkImpl = fork } = {}) {
  ...
  function importCheckpoint(originalName, bytes) {
    ...
    return {
      ...
      encodingVersion: Number(payload.encoding && payload.encoding.state) ||
        Number(payload.model && payload.model.encodingVersion) || null,
      encodingCompatible: Number(payload.encoding && payload.encoding.state) === STATE_ENCODING_VERSION ||
        Number(payload.model && payload.model.encodingVersion) === STATE_ENCODING_VERSION,
  ...
  function compileNativeWorker(config, outputFile, backend = 'python-binary') {
    ...
    if (!compilerPath) throw new Error('找不到 C++ 编译器：请安装 clang++/g++，或设置 CXX/LLVM_ROOT 后重启服务');
    // 先落到 .tmp 再原子改名：编译失败时不要把半个 exe 留在正式路径上，
    // 否则下一次会因为「产物已存在且比源码新」跳过编译而一直用到坏产物。
    const tempFile = outputFile + '.tmp';
    // 与服务端 worker 保持一致：LibTorch 模板编译默认使用低内存优化级别，
    // 避免在小内存服务器上被 cc1plus 的峰值内存占用杀死。
    ...
    if (backend === 'libtorch') {
      ...
      if (!fs.existsSync(include) || !fs.existsSync(lib)) {
        throw new Error('未找到 LibTorch：请设置 CITADELS_LIBTORCH，或在项目虚拟环境/python3 中安装 torch');
      }
    ...
    args.push(path.join(root, 'native', 'mcts_worker.cpp'), '-o', tempFile, ...linkArgs);
    ...
    return new Promise((resolve, reject) => {
      ...
      compiler.on('error', error => reject(new Error('启动 C++ 编译器失败：' + error.message)));
  ...
  async function prepareNativeWorker(config) {
    ...
    if (backend === 'libtorch' && (!builtAt || sourceAt > builtAt)) {
      await ensureTorchRoot(root, text => appendLog('[native] ' + text));
    }
    if (!builtAt) {
      ...
      if (process.platform !== 'win32') {
        try { fs.chmodSync(outputFile, 0o755); }
        catch (error) { throw new Error('设置 ' + path.basename(outputFile) + ' 可执行权限失败：' + error.message); }
      }
    }
    return outputFile;
  }
  ...
  function start(rawConfig) {
    ...
    if (config.resumeCheckpoint) {
      ...
      const checkpoint = checkpointConfig(config.resumeCheckpoint);
      if (checkpoint.encodingVersion !== STATE_ENCODING_VERSION) {
        throw new Error('该 checkpoint 使用状态编码 v' + (checkpoint.encodingVersion || '未知') +
          '，当前训练需要 v' + STATE_ENCODING_VERSION + '；请选择新权重或从头训练');
      }
    }
    ...
    if (rawConfig && rawConfig.mctsSimulations != null) {
      appendLog('MCTS 配置：模拟=' + config.mctsSimulations + ' · c_puct=' + config.mctsC_puct +
        ...
        ' · 评估器=' + (config.mctsEvaluator || 'native') +
