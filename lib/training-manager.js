...
function createTrainingManager({ root = path.join(__dirname, '..'), forkImpl = fork } = {}) {
  ...
  function nativeWorkerPath() {
    return path.join(root, 'native', process.platform === 'win32' ? 'search_worker.exe' : 'search_worker');
  }

  function compileNativeWorker(config, outputFile) {
    const args = ['-std=c++20', '-O2', '-I', path.join(root, 'native'), path.join(root, 'native', 'search_worker.cpp'), '-o', outputFile];
    if (config.neuralNetworkFramework === 'libtorch') {
    ...
    appendLog('开始编译 search_worker.exe：clang++ ' + args.join(' '));
    return new Promise((resolve, reject) => {
      const compiler = spawn(process.env.CXX || 'clang++', args, { cwd: root, windowsHide: true, stdio: ['ignore', 'pipe', 'pipe'] });
      ...
      compiler.on('exit', (code, signal) => code === 0
        ? resolve(outputFile)
        : reject(new Error('search_worker.exe 编译失败（code=' + code + ', signal=' + (signal || 'none') + '）')));
    });
  }

  async function prepareNativeWorker(config) {
    const outputFile = nativeWorkerPath();
    status.prepareMessage = '检查 search_worker.exe…';
    appendLog('检查 C++ 搜索进程：' + outputFile);
    if (!fs.existsSync(outputFile)) {
      status.prepareMessage = '正在编译 search_worker.exe…';
      appendLog('未找到编译产物，准备使用 clang++ 编译 search_worker.exe');
      await compileNativeWorker(config, outputFile);
      appendLog('search_worker.exe 编译完成');
    } else {
      status.prepareMessage = '已找到 search_worker.exe，准备启动…';
      appendLog('已找到 search_worker.exe，跳过编译');
    }
    return outputFile;
  }
  ...
  function start(rawConfig) {
    ...
    const launch = workerPath => {
      ...
      status.prepareMessage = workerPath ? 'search_worker.exe 已启动，正在启动训练…' : '';
      appendLog(workerPath ? '启动 search_worker.exe：' + workerPath : '启动训练进程');
    ...
    const needsNative = config.mctsEngine === 'cpp' || config.backend === 'native';
    if (needsNative) {
      preparing = true;
  ...
  function stop() {
    if (preparing && status.running) {
      status.stopping = true; status.state = 'stopping'; status.prepareMessage = '正在停止 search_worker 准备流程…';
      appendLog('收到停止请求，取消 search_worker 准备流程');
      return snapshot();
    }
