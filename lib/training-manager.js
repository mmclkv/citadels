...
function createTrainingManager({ root = path.join(__dirname, '..'), forkImpl = fork } = {}) {
  ...
  function nativeWorkerPath() {
    return path.join(root, 'native', process.platform === 'win32' ? 'mcts_worker.exe' : 'mcts_worker');
  }
  ...
  function compileNativeWorker(config, outputFile) {
    ...
    const args = ['-std=c++20', '-O2', '-I', path.join(root, 'native'), path.join(root, 'native', 'mcts_worker.cpp'), '-o', outputFile];
    ...
    appendLog('开始编译 mcts_worker.exe：' + compilerPath + ' ' + args.join(' '));
    return new Promise((resolve, reject) => {
      ...
      compiler.on('exit', (code, signal) => code === 0
        ? resolve(outputFile)
        : reject(new Error('mcts_worker.exe 编译失败（code=' + code + ', signal=' + (signal || 'none') + '）')));
    });
  }

  async function prepareNativeWorker(config) {
    const outputFile = nativeWorkerPath();
    status.prepareMessage = '检查 mcts_worker.exe…';
    appendLog('检查 C++ 搜索进程：' + outputFile);
    if (!fs.existsSync(outputFile)) {
      status.prepareMessage = '正在编译 mcts_worker.exe…';
      appendLog('未找到编译产物，准备使用 clang++ 编译 mcts_worker.exe');
      await compileNativeWorker(config, outputFile);
      appendLog('mcts_worker.exe 编译完成');
    } else {
      status.prepareMessage = '已找到 mcts_worker.exe，准备启动…';
      appendLog('已找到 mcts_worker.exe，跳过编译');
    }
    return outputFile;
  }
  ...
  function start(rawConfig) {
    ...
    const launch = workerPath => {
      ...
      status.prepareMessage = workerPath ? 'mcts_worker.exe 已启动，正在启动训练…' : '';
      appendLog(workerPath ? '启动 mcts_worker.exe：' + workerPath : '启动训练进程');
