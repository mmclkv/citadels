...
粒子不再是均匀的：调用方可以随池带一份 `particleWeights`（信念权重，见
`training/belief.js`），`mcts_worker` 原样递给 `search()`，每条模拟按权抽样。
**粒子被 `actions_aligned` 丢掉时权重必须同步丢掉**，否则权重会整体错位套到别的
世界上 —— 比没有信念更糟。响应里的 `belief` 如实回报是否真的按权重抽了样。
