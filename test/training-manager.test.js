...
assert.ok(status.logFile && fs.existsSync(status.logFile), '开始训练会创建日志文件');
child.emit('message', { type: 'log', text: '日志文件写入测试' });
assert.match(fs.readFileSync(status.logFile, 'utf8'), /日志文件写入测试/, '训练事件会实时写入日志文件');
